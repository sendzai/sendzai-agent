...
#### E. Scheduling Status Updates with Media
```bash
# Schedule a WhatsApp status with a media attachment
...
#### F. Scheduling a Message with Multiple Media Attachments
```bash
# Use --media <type>:<url> (repeatable) to attach multiple images or videos
...
#### B. Single Media Status (Image/Video)
Use `--media <type>:<url>` to attach a media item:
```bash
sendzai post-status -m "Enjoying a beautiful morning!" \
  --media image:https://picsum.photos/seed/morning/800/600
```

#### C. Multiple Media Statuses (Posted Sequentially)
Pass `--media` multiple times — each item is sent as a separate story with a 1-second delay:
```bash
...
// 2. Post a multi-media status update
