...
function collectMedia(value: string, previous: MediaItem[]): MediaItem[] {
  const idx = value.indexOf(":");
  if (idx === -1) return previous;
  const type = value.substring(0, idx) as "image" | "video";
...
program
  ...
  .action((options) => {
    ...
    if (options.server) updates.apiUrl = options.server;
    saveConfig(updates);
...
program
  ...
  .option("--media <type:url>", "Media attachment (repeatable, e.g. --media image:https://site/a.jpg)", collectMedia, [] as MediaItem[])
