...
async function handleToolCall(fn: () => Promise<any>, mapper?: (res: any) => any) {
  try {
    let result = await fn();
    if (mapper) {
    ...
    return {
      isError: true,
      content: [{ type: "text" as const, text: JSON.stringify({ success: false, error: errMsg }, null, 2) }],
    };
  }
}
...
server.registerTool(
  ...
  async (args) => {
    return handleToolCall(
      () => client.listRecipientLists(args.query),
      (res) => ({ lists: res })
    );
  }
);
...
server.registerTool(
  ...
  async (args) => {
    return handleToolCall(
      () => client.searchContacts(args.query, args.listId),
      (res) => ({ contacts: res })
    );
  }
);
...
server.registerTool(
  ...
  async (args) => {
    return handleToolCall(
      () => client.searchGroups(args.query, args.deviceId),
      (res) => ({ groups: res })
    );
  }
);
